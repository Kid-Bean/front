...
class UpdateKidInfoActivity : AppCompatActivity() {
    ...
    private fun setMemberInfo(body: MemberInfoResponse?) {
        body?.memberGender?.let { gender ->
            when (gender) {
                "MAN" -> {
                    this.gender = gender
                    updateButtonStyles(binding.btnBoy, binding.btnGirl)
                }
                "WOMAN" -> {
                    this.gender = gender
                    updateButtonStyles(binding.btnGirl, binding.btnBoy)
                }
                else -> {
                }
            }
        ...
        val split = body?.memberBirth?.split("-")
        if (split != null) {
            binding.etYear.setText(split[0])
            binding.etMonth.setText(split[1])
            binding.etDay.setText(split[2])
        }

        if (body?.memberName != null) {
            binding.etName.setText(body.memberName)
        }
        ...
        homeController.uploadMemberInfo(MemberInfoRequest(name, gender, birthDate)).enqueue(object :
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    // 정상적으로 통신이 성공된 경우
                    Log.d("post", "onResponse 성공: " + response.body().toString())
                    Toast.makeText(this@UpdateKidInfoActivity, "정보가 수정되었습니다.", Toast.LENGTH_SHORT)
                        .show()

                } else {
                    // 통신이 실패한 경우(응답코드 3xx, 4xx 등)
                    Toast.makeText(this@UpdateKidInfoActivity, "수정이 실패하였습니다.", Toast.LENGTH_SHORT)
                        .show()
                    Log.d("post", "onResponse 실패 + ${response.code()}")
                }

                // MypageActivity로 이동
                val intent = Intent(this@UpdateKidInfoActivity, MypageActivity::class.java)
